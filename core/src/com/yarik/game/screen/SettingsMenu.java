...
/**
 * Created by Ярослав on 25.11.2014.
 */
...
public class SettingsMenu implements Screen {
    ...
    public static FileHandle levelDirectory() {
        String prefsDir = Gdx.app.getPreferences(SOTGame.TITLE).getString("leveldirectory").trim();
        if(prefsDir != null && !prefsDir.equals(""))
            return Gdx.files.absolute(prefsDir);
        else
            return Gdx.files.absolute(Gdx.files.external(SOTGame.TITLE + "/levels").path()); // return default level directory
    }

    /** @return if vSync is enabled */
    public static boolean vSync() {
        return Gdx.app.getPreferences(SOTGame.TITLE).getBoolean("vsync");
    }
        ...
        ClickListener buttonHandler = new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                ...
                if(event.getListenerActor() == vSyncCheckBox) {
                    // save vSync
                    Gdx.app.getPreferences(SOTGame.TITLE).putBoolean("vsync", vSyncCheckBox.isChecked());

                    // set vSync
                    Gdx.graphics.setVSync(vSync());

                    Gdx.app.log(SOTGame.TITLE, "vSync " + (vSync() ? "enabled" : "disabled"));
                } else if(event.getListenerActor() == back) {
                    // save level directory
                    String actualLevelDirectory = levelDirectoryInput.getText().trim().equals("") ? Gdx.files.getExternalStoragePath() + SOTGame.TITLE + "/levels" : levelDirectoryInput.getText().trim(); // shortened form of an if-statement: [boolean] ? [if true] : [else] // String#trim() removes spaces on both sides of the string
                    Gdx.app.getPreferences(SOTGame.TITLE).putString("leveldirectory", actualLevelDirectory);

                    // save the settings to preferences file (Preferences#flush() writes the preferences in memory to the file)
                    Gdx.app.getPreferences(SOTGame.TITLE).flush();

                    Gdx.app.log(SOTGame.TITLE, "settings saved");
